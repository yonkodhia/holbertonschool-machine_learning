...
class DeepNeuralNetwork:
    ...
    def weights(self):
        ...
        self.__cache["A0"] = X
        for law in range(self.__L):
            weights = self.__weights["W{}".format(law + 1)]
            e = self.__cache["A{}".format(law)]
            biases = self.__weights["b{}".format(law + 1)]

            H = np.matmul(weights, e) + biases

            resultes = 1 / (1 + np.exp(-H))

            self.__cache["A{}".format(law + 1)] = resultes

        return self.__cache["A{}".format(law + 1)], self.__cache
